...
act_labels <- read.csv("D:/l4/activity_labels.txt", header=FALSE, dec=".",sep=" ")
colnames(act_labels) <- c("n", "activity")

#Reading test,train data
test_lines <- readLines(file("D:/l4/X_test.txt",open="r"))
train_lines <- readLines(file("D:/l4/X_train.txt",open="r"))
data_lines<-c(test_lines,train_lines)

#Reading activity numbers
test_act_nbs <- readLines(file("D:/l4/y_test.txt",open="r"))
train_act_nbs <- readLines(file("D:/l4/y_train.txt",open="r"))
